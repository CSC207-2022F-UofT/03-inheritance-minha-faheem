...
class BagMain {
    ...
    public static void enhanceBags(Bag[] bags, boolean double_enhance_handbags) {
        // TODO: Implement this.
        for (Bag thisBag : bags) {
            if (double_enhance_handbags && thisBag instanceof HandBag) {
                thisBag.enhance();
                thisBag.enhance();
            } else {
                thisBag.enhance();
    ...
    public static int countCrossbodyStraps(Bag[] bags) {
        ...
        int totalStraps = 0;
        for (Bag thisBag : bags) {
            if (thisBag instanceof CrossbodyBag) {
                totalStraps = totalStraps + ((CrossbodyBag)thisBag).getNumberOfStraps();
            }
        }
        return totalStraps;
    }
